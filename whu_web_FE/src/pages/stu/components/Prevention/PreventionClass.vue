...
export default {
  ...
  data () {
    return {
      ...
      'cure': '肌肉拉伤后，要立即进行冷敷，用冷水冲局部或用毛巾包裹冰决冷敷，然后用绷带适当用力包裹损伤部位，防止肿胀。24小时至48小时后拆除包扎，可外贴活血和消肿胀，可适当热敷或用较轻的手法对损伤局部进行按摩。\n' +
      ...
      binjianyan: {
        'name': '髌腱炎',
        'brief': '髌腱是指连接你的髌骨 (即膝盖骨) 与小腿胫骨之间的肌腱。当它受损或出现炎症时，称为髌腱炎 (Patellar tendinitis)。',
        'reason': '髌腱炎是一种常见的因髌腱使用过度而导致的损伤。即当你反复施加压力于髌腱上时，刚开始会导致肌腱发生微小的撕裂，这时，你的身体会试图修复这些损伤。但当肌腱的撕裂越来越多时，你的身体无法及时修复，最终导致越来越严重的肌腱损伤和炎症。',
        'behave': '患上髌腱炎后，首先出现的症状是髌腱部位疼痛。这种疼痛的特征可能表现为：\n' +
          '\n' +
          '起初只在你运动时或在大量运动后有疼痛。\n' +
          '当加大运动强度时，疼痛也会加重。\n' +
          '随着病情的进展，不管是否运动，都有可能有疼痛。\n' +
          '因为疼痛，上下楼变得比较困难。\n' +
          '当病情恶化，疼痛变得持续时，可能会使你在夜间难以入睡。',
        'cure': '大部分髌腱炎患者采用保守 (即非手术) 治疗后，疼痛会得到缓解，病情也会逐渐好转。如果你的症状一直没有改善，持续了六个月甚至更长，那么你应该和医生讨论一下是否应该进行手术治疗，当然，在决定手术治疗之前，你应该了解手术治疗的好处和风险。',
        'prevend': '多休息：休息并不意味着停止所有的活动，而是要避免跑步、跳跃等活动。如果出现疼痛，不要忍着继续活动，以免进一步损伤你的肌腱。\n' +
            '冰敷：冰敷的方法有很多，比如你可以将冰块放在塑料袋中，并用毛巾包好，放在疼痛处。你也可以将水放在塑料杯中冷冻，然后用这个冷冻杯子贴着疼痛处滚动按摩。冰敷可以帮助缓解你的疼痛，减轻肿胀。\n' +
            '掌握安全运动的技巧：你可以向理疗师或运动专家学习在活动中如何更好地掌握力量的使用。例如，一个经常需要跳跃的运动员应该学习正确的起跳与着地的技巧。\n' +
            '多做伸展运动：这是因为当肌肉 (特别是大腿的股四头肌) 僵硬时，容易造成髌腱的劳损。\n' +
            '增强肌腱：理疗师可能会教你一些运动方法来增强你的髌腱以及周围的肌肉，包括股四头肌。\n' +
            '使用髌腱绷带：这种绷带可以帮助髌腱分散所承受的压力，缓解疼痛。'
      },
      banyueban: {
        'name': '半月板撕裂',
        'brief': '半月板撕裂 (Torn meniscus) 是指你膝盖中的一个或两个半月形的软骨 (下图中的红色部位) 发生了损伤。\n',
        'reason': '任何需要你有力地扭动或旋转膝盖的活动，包括猛烈地旋转、突然停止或转弯，都可能造成半月板撕裂。即便是跪着、深蹲或提重物有时也会导致半月板撕裂。在老年人中，因为膝盖的退行性变化，更容易发生半月板撕裂。\n' +
            '\n' +
            '在运动员中，半月板撕裂可能还伴随其他的损伤，例如膝盖的前交叉韧带撕裂。',
        'behave': '如果你的半月板发生了撕裂，你的膝盖可能会出现以下的一些症状：\n' +
            '\n' +
            '膝盖有 “弹出感”。\n' +
            '肿胀或僵直。\n' +
            '疼痛，特别是扭动或旋转你的膝盖时。\n' +
            '难以完全伸直你的膝盖，或是在膝盖活动时有阻滞的感觉，好像膝盖 “上了锁”。\n' +
            '另外，在行走或承重时也可能会有疼痛',
        'cure': '半月板撕裂的治疗一般开始时以保守为主。医生可能会建议你：\n' +
            '\n' +
            '休息： 避免会加剧你膝盖疼痛的活动，特别是可能会引起你膝盖扭曲的活动。在行走时，你可能需要使用拐杖来减轻膝盖的压力，以促进愈合。\n' +
            '冰敷： 冰敷可以减轻膝盖的疼痛和肿胀。在受伤后的头一两天每 4-6 小时冰敷一次，以后可以按需要使用。每次冰敷时可使用冰袋或冰毛巾敷 15 分钟。\n' +
            '药物治疗： 如果需要，可以服用非处方类镇痛药以帮助缓解膝盖的疼痛。\n' +
            '膝盖理疗： 物理疗法可以帮助强化、稳定你的膝盖及其周围的肌肉。\n' +
            '脚部支撑物： 脚部支撑物或鞋内垫可以帮助更平衡地分散膝盖周围的力量，或是减轻膝盖某些特定部位的压力。',
        'prevend': '带护膝，半月板损伤的机制主要是出现在膝关节屈曲、内收、旋转动作下，会出现膝关节的半月板损伤，干什么工作会出现膝关节的这样的姿势呢？最常见的就是踢足球的足球运动员，在踢足球的过程中，膝关节处于这样的姿势，也就是膝关节的屈曲、内收再加上旋转的力量，在踢球运动的时候膝关节一定要佩戴好护膝。护膝的主要作用就是加强膝关节、韧带的作用，在膝关节做高难度动作时能够保持膝关节的稳定性，对膝关节的韧带及半月板都有一定的保护作用。'
      }
    }
  },
  mounted () {
    ...
    if (this.title === '髌腱炎') {
      this.items = this.binjianyan
    }
    if (this.title === '半月板撕裂') {
      this.items = this.banyueban
    }
